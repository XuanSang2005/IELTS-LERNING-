        ...
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between md:gap-10">
          <div>
            ...
            <p className="mt-3 max-w-[52ch] font-fraunces text-[18px] italic leading-relaxed text-graphite md:text-[19px]">
              Four arcs, twelve weeks each. Begin where your band actually sits — the rest is an
              ornament your writing cannot yet carry.
            </p>
          </div>
          <p className="font-mono text-[10px] uppercase tracking-[0.28em] text-graphite">
            CURRENT · {GRAMMAR_LEVELS.find((l) => l.level === value)?.label.toUpperCase()}
          </p>
        </div>

        <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
          {GRAMMAR_LEVELS.map((meta) => {
